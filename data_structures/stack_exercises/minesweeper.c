...
void uncover(Cell **field, int nrows, int ncols, int x, int y)
{
  ...
  if (!field[x][y].visible)
  {
    ...
    push(s, (point){x, y}); // Empilhar a posição escolhida pelo jogador.
    while (!empty(s)){ // enquanto a pilha não estiver vazia.
      point p = pop(s); // desempilhe!
      if(field[p.x][p.y].data == ' ')
        // verifique os 8 vizinhos.
          // Se nenhuma das verificações forem verdadeiras:
              // Empilhe essa posição vizinha e a marque como visível para o jogador.
    }
    free(s);
  }
}
